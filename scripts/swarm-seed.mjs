...
import BeeClient from "@ethersphere/bee-js"
...
const bee = new BeeClient.Bee(new URL(process.env.VITE_APP_GATEWAY_URL).origin)

export async function testSeed() {
  const batchId = "b84f43bd8f1e1f53a2c777546a03872df6537e4e9f1aff4a43bc983b93628fc7"

  try {
    const resp = await bee.uploadFile(
      batchId,
      JSON.stringify({ test: "Ciao" })
    )
    console.log("reference", resp.reference)
    const topic = bee.makeFeedTopic("Test")
    const feed =
      bee.makeFeedWriter("sequence", topic, "f7622a4bd4375046bbc9712e00ecebdf9460a1212b72c585c4096c3bc80f7634")
    const updateReference = await feed.upload(batchId, resp.reference)
    const feedManifest =
      await bee.createFeedManifest(batchId, "sequence", topic, "0xa6Ac1c7b69Ae39Ba85e803F6A3639c90D774B414")
