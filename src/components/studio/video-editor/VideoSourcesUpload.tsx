...
const VideoSourcesUpload = React.forwardRef<VideoSourcesUploadHandlers, VideoSourcesUploadProps>(
  ({ initialDragPortal, disabled, onComplete, onCancel }, ref) => {
              ...
              {({ isUploading }) => (
                <VideoSourcePreview
                  ...
                  actionsRender={
                    <>
                      {i > 0 && !isUploading && (
                        <Button aspect="text" onClick={() => removeSource(i)} disabled={disabled}>
                          Remove
                        </Button>
                      )}
                      {isUploading && (
                ...
                >
                  {isUploading && <FileUploadProgress progress={currentQueue?.completion ?? 0} />}

                  {!isUploading && !finished && <p>Waiting to upload...</p>}
